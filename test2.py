...
class NeuralNetNodeCnn(NeuralNetNode):
    """
    """
    def _init_node_parm(self, conf_data):
    ...
    def run(self, conf_data):
        ...
        if self.net_type == "resnet":
            self.get_model_resnet(self.netconf, "T")
            test_data, dataconf = self.get_input_data(self.feed_node, self.cls_pool, self.eval_feed_name)
            self.return_arr = self.train_run_resnet(input_data, test_data)
        else:
            self.get_model_cnn(self.netconf, "T")
            self.return_arr = self.train_run_cnn(input_data)

            self.return_data["TrainResult"] = self.return_arr
